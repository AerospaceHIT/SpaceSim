...
SAT_1为无漂移冻结轨道，近地点幅角为90度；SAT_2漂移5度，所以近地点幅角为85度；SAT_3漂移5度，所以近地点幅角为85度；SAT_3漂移25度，所以近地点幅角为65度；SAT_4漂移90度，所以近地点幅角为0度。
...
（1）熟悉SpaceSim添加导弹和地面站的操作流程，熟悉利用指令实现导弹攻击地面目标，以及输出导弹相关参数。
...
在SpaceSim平台完成导弹攻击模拟仿真任务，导弹从中国境内（经度87°，纬度42°）飞行至美国境内（经度-77°，纬度38.9°），生成仿真动画并观察导弹的飞行弹道。
...
（1）如果是导弹攻击任务，则任务类型选择“导弹”，为了显示直观，可在打击地点处添加“地面站”对象以作标记；

（2）如果是防御拦截任务，则任务类型选择“快速交会”，可添加“卫星”对象，与卫星相关仿真相同；
...
在近地圆轨道上存在一颗卫星，轨道高度500 km，轨道倾角40°，现要发射一颗导弹击毁卫星。导弹位于中国境内，经度88°，纬度39°，通过SapceSim完成弹道计算和仿真过程。
...
本次仿真任务实现了导弹拦截卫星从弹道计算、发射到打击全过程，达成了实验目标。
...
在中国境内某处（经度100.298°，纬度40.961°）发射一枚火箭，将卫星送入轨道高度500 km，轨道倾角98°的近地圆轨道上。通过SapceSim完成计算和仿真过程。
...
本次仿真任务实现了运载火箭拦截卫星从弹道计算、发射到入轨的过程，基本达成了实验目标。
...
| -------------- | ------------ | ------------ | -------- |
| 近地点高度(km) | 5000         | 20000        | 42166.3  |
| 远地点高度(km) | 5000         | 20000        | 0        |
| 轨道倾角(°)    | 98           | 98           | 50       |
| 升交点赤经(°)  | 0            | 20           | -80      |
| 近地点幅角(°)  | 0            | 0            | 0        |
| 平近点角(°)    | 0            | 0            | 0        |
...
在场景中添加一组Walker星座（详细参数见“实验内容”部分），并为星座中每一颗卫星配置锥角为5°的圆锥波束天线。
...
打开SpaceSim，点击“新建场景”，输入场景名称，由于卫星的升交点赤经值由仿真时间计算所得，需要将仿真时间与案例中统一，仿真开始时刻设置为2017/1/1_12:00:00，仿真结束时间设置为2017/1/1_12:00:00。
...
在场景中添加3颗卫星，轨道参数如表 37-1所示，将轨道模型修改为J2摄动。
...
如图 38-1所示，在卫星设置界面，勾选“轨道数据文件”，在后方的选项栏中，选择飞行器类型为“飞机”，完成后点击“加载文件”，进入SpaceSim的安装目录，进入下列文件夹：
